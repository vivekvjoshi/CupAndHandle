...
def find_cup_and_handle(df, back_days=365):
    ...
    for r_idx in max_idx[::-1][:3]:
        ...
        for l_idx in max_idx:
            ...
            # --- Win Probability / Quality Score (0-100) ---
            # Heuristic based on ideal traits
            score = 60 # Base score for just existing
            
            # 1. Handle Tightness (The flatter the handle, the more explosive the breakout)
            # Retracement < 10% is good. < 5% is excellent.
            retracement_pct = (handle_drop / right_rim_price) * 100
            if retracement_pct < 5:
                score += 15
            elif retracement_pct < 10:
                score += 10
                
            # 2. Cup Depth (Ideal: 15% - 35%)
            cup_pct_val = cup_depth_pct * 100
            if 15 <= cup_pct_val <= 35:
                score += 10
            elif cup_pct_val < 10: # Too shallow
                score -= 5
            elif cup_pct_val > 50: # Too deep (volatile)
                score -= 10
                
            # 3. Handle Duration (Ideal: > 1 week, < 4 weeks?)
            if 7 <= days_since_right_rim <= 25:
                score += 5
